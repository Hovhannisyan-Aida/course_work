...
class GatewayTester:
        ...
        for i in range(limit + 10):
            try:
                response = await self.client.get(
                    ...
                    headers={
                        "X-User-ID": user_id,
                        "X-User-Tier": "anonymous"  # Explicitly set anonymous tier (50 req/min)
                    }
                )
    ...
    async def test_caching(self):
        ...
        # Use unique query parameter to avoid hitting old cache
        ...
        try:
            ...
            # Second request with same URL (should be cache hit)
    ...
    async def test_circuit_breaker(self):
        ...
        # Սա ավելի լավ կաշխատի եթե backend service-ը fail է լինում
        # Մեր դեպքում ընդամենը կստուգենք որ circuit breaker-ը գոյություն ունի
    ...
    async def run_all_tests(self):
        ...
        tests = [
            ...
            self.test_headers,
            self.test_rate_limiting,  # Last because it sends many requests
        ]
